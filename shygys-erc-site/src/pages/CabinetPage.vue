  ...
  <q-page class="cabinet-page q-pa-md">
    ...
    <q-dialog v-model="dialogAddZayav">
      <q-card style="min-width: 320px" class="q-elevation-6">
        ...
        <q-card-actions align="right" class="button-actions">
          <q-btn color="grey" :label="t('cancel')" @click="closeAddZayav" />
          <q-btn color="primary" :label="t('confirm')" @click="saveIsMailing" />
        </q-card-actions>
      </q-card>
    </q-dialog>
                    ...
                    >
                      ...
                      >
                        ...
                        <tbody>
                          <tr v-for="row in provider.Rows || []" :key="row.ServiceName">
                            ...
                            <td>
                              <div v-for="(pok, idx) in row.PokRow || []" :key="'pay' + idx">
                                {{ pok.PayCount }}
                              </div>
                            </td>

                            <td>
                              <div v-for="(pok, idx) in row.PokRow || []" :key="'check' + idx">
                                {{ pok.CheckCount }}
                              </div>
                            </td>

                            <td>
                              <div v-for="(pok, idx) in row.PokRow || []" :key="'cons' + idx">
                                {{ pok.Consumption }}
                              </div>
                            </td>

                            <td>
                              <div v-for="(pok, idx) in row.PokRow || []" :key="'tariff' + idx">
                                {{ pok.Tariff }}
                              </div>
                            </td>
                      ...
                      <div v-else class="billing-cards">
                        ...
                        >
                          ...
                          <div class="card-field">
                            <strong>{{ t('saldoNach') }}:</strong> {{ row.SumSaldoBegin }}
                          </div>
                          <div class="card-field">
                            <strong>{{ t('oplata') }}:</strong> {{ row.SumPay }}
                          </div>
                          ...
                          <div class="card-field">
                            <strong>{{ t('nach') }}:</strong> {{ row.SumCalc }}
                          </div>
                          <div class="card-field">
                            <strong>{{ t('penya') }}:</strong> {{ row.SumPen }}
                          </div>
                          <div class="card-field">
                            <strong>{{ t('pereraschet') }}:</strong> {{ row.SumRecalc }}
                          </div>
                          <div class="card-field">
                            <strong>{{ t('toOplata') }}:</strong> {{ row.SumSaldoEnd }}
                          </div>
                        </div>
                      </div>
                    </div>
      ...
      <q-card class="settings-card shadow-2 q-mb-xl" style="margin-top: 20px">
        ...
        <q-card-section>
          <div class="row items-center justify-between q-mb-md" style="color: #123046">
            ...
            <div class="button-actions">
              <q-btn no-caps color="primary" :label="t('save')" @click="dialogSaveIsMailing" />
            </div>
          </div>
          <q-separator />
          <div
...
export default defineComponent({
  setup(props, { emit }) {
    ...
    const dialogAddZayav = ref(false)
    ...
    const dialogSaveIsMailing = () => {
      if (isM.value) {
        dialogAddZayav.value = true
      } else {
        saveIsMailing()
      }
    }
